...
from tf.transformations import quaternion_multiply, quaternion_inverse, quaternion_conjugate, quaternion_matrix
...
class MavrosVisionPoseNode(Node):
    def __init__(self):
        ...
        self.create_timer(FREQ_0_5_HZ, self.publish_initial_pose)
        ...
        self.create_timer(FREQ_0_5_HZ, self.publish_vicon_initial_pose)
        ...
        self.create_timer(FREQ_10_HZ, self.publish_cam_to_vicon_tf)
        
        # Set up subscribers
        self.initial_vicon_pose_sub = self.create_subscription(
            PoseStamped,
    ...
    def vicon_callback(self, msg):
        ...
        # Update pose(s)
        # Let initial Vicon pose be set in initial_vicon_pose_callback
    ...
    def calculate_cam_to_vicon_tf(self):
        ...
        if self.initial_camera_pose is None or self.initial_vicon_pose is None:
            return None
        ...
        q_c = self.initial_camera_pose.pose.orientation
        p_c = self.initial_camera_pose.pose.position
    ...
    def initial_vicon_pose_callback(self, msg):
        ...
        if self.initial_vicon_pose is None:
            self.initial_vicon_pose = current_pose
            self.get_logger().info(f"Vicon - Set initial pose: x={self.initial_vicon_pose.pose.position.x}, y={self.initial_vicon_pose.pose.position.y}, z={self.initial_vicon_pose.pose.position.z}")
    ...
    def publish_vicon_initial_pose(self):
        if self.initial_vicon_pose is None:
            ...
            self.vicon_ego_init_pub.publish(self.initial_vicon_pose)
    ...
    def publish_cam_to_vicon_tf(self):
        if self.cam_to_vicon_tf is None:
            ...
            tf_msg = self.calculate_cam_to_vicon_tf()
            if tf_msg is None:
                return
